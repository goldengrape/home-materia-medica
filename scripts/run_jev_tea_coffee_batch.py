#!/usr/bin/env python3
"""Run Jev v0.4 on ten frozen tea/coffee summaries; preserve raw outputs unchanged."""
...
RESULTS = ROOT / "qa/jev-tea-coffee-batch/raw"
PRE_FREEZE = ROOT / "qa/jev-tea-coffee-batch/pre-freeze"
...
def main() -> int:
    ...
    v04 = load_module(EXPERIMENT / "run_pilot_v04.py", "jev_v04_tea_coffee_batch")
    ...
    # Verify every stage-2 snapshot before the first API call.
    cases = []
    for digest, path in manifest:
        entry_id = path.parent.name
        snapshot = json.loads((PRE_FREEZE / f"{entry_id}.json").read_text(encoding="utf-8"))

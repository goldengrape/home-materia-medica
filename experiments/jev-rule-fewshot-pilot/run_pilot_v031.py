...
FIXTURE_PATH = ROOT / "fixtures-v0.3.1.json"
RESULTS_PATH = ROOT / "results-v0.3.1.json"
...
由于本项目输出层只评估五脏，六腑相关症候先进入相表里的五脏功能群：胆并入肝胆功能群后支持肝；胃并入脾胃功能群后支持脾；大肠并入肺与大肠功能群后支持肺；小肠并入心与小肠功能群后支持心；膀胱并入肾与膀胱功能群后支持肾。三焦不做固定机械投影，应根据上焦、中焦、下焦的具体症候再判断相关五脏。这只是本项目五字段输出的工作投影，不表示六腑归经与五脏归经在传统理论中等同。
...
def api_json(method: str, path: str, api_key: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "home-materia-medica-jev-pilot/0.3.1",
        },
    )
    try:
